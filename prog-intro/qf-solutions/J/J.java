...
public class J {
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(System.in)) {
            ...
            for (int i = 0; i < n; i++) {
                line = sc.next();
                for (int j = 0; j < n; j++) {
                    mas[i][j] = Character.getNumericValue(line.charAt(j));
                }
            }
    ...
    static void substract(int[] mas1, int[] mas2) {
        int n = mas1.length;
        for (int i = 0; i < n; i++) {
            mas1[i] = substract(mas1[i], mas2[i]);
        }
    }

    static int substract(int a, int b) {
        return a - b >= 0 ? a - b : 10 + a - b;
    }
