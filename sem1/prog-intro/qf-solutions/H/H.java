    ...
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(System.in)) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                maxTransaction = Math.max(transactions[i], maxTransaction);
                sum[i + 1] = sum[i] + transactions[i];
            }
            //System.err.println(Arrays.toString(transactions));
            //System.err.println(Arrays.toString(sum));
            //System.err.println(maxTransaction);
            int[] queries = new int[sum[n]];
            for (int i = 0; i < n; i++) {
                for (int j = sum[i]; j < sum[i + 1]; j++) {
                    queries[j] = i;
                }
            }
            //System.err.println(Arrays.toString(queries));
            int[] numberOfBatches = new int[sum[n] + 1];
            for (int t = 0; t < maxTransaction; t++) {
                numberOfBatches[t] = -1; //Impossible
            }
            int counter, index;
            for (int t = maxTransaction; t < sum[n] + 1; t++) {
                counter = 0;
                //System.err.println("t = " + t);
                int i = -1; // index of the last transaction in the previous query
                while (true) {
                    counter++;
                    index = sum[i + 1] + t;
                    if (index >= sum[n]) {
                ...
                numberOfBatches[t] = counter;
            }
            //System.err.println(Arrays.toString(numberOfBatches));
            int numberOfQueries = sc.nextInt();
            for (int i = 0; i < numberOfQueries; i++) {
                int q = sc.nextInt();
                System.out.println(numberOfBatches[q] == -1 ? "Impossible" : numberOfBatches[q]);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
