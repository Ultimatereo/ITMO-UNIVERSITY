...
//            return deepcopy(knownRecipes.get(index));
//        }
        ...
        if (reason.charAt(1) == 'D') {
            ...
            return constructDeduction(index, number, statements);
...
//        return deepcopy(knownRecipes.get(index));
    ...
    private static ArrayList<ProofItem> deepcopy(ArrayList<ProofItem> proofItems) {
        ArrayList<ProofItem> copy = new ArrayList<>();
        for (ProofItem pi : proofItems) {
            copy.add(new ProofItem(pi.rawExpression, pi.reason));
        }
        return copy;
    }
