		...
		for (int i = 0; i < n; i++) {
			...
			if (key == 1) {
				...
				if (size == values.length) {
					values = expandArraySize(values);
					min = expandArraySize(min);
				}
				...
				if (size == 0) {
					...
					min[size] = Math.min(min[size - 1], value);
				}
				size += 1;
			} else if (key == 2) {
				if (size == values.length / 2) {
					values = broadenArraySize(values);
					min = broadenArraySize(min);
				}
				size -= 1;
				values[size] = Integer.MAX_VALUE;
			} else {
				output.write(String.valueOf(min[size - 1]));
				output.write('\n');
			}
			//System.out.println(Arrays.toString(values) + " " + size + " " + min);
			input.skipAllLine();
		}
		input.close();
		output.close();
