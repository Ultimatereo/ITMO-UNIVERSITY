...
public class RMQ {
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(System.in)) {
            ...
            DO derevo = new DO(length, mas);
            while (sc.hasNext()) {
                String command = sc.next();
                if (command.equals("min")) {
                    System.out.println(derevo.min(sc.nextInt() - 1, sc.nextInt() - 1));
                } else {
                    derevo.set(sc.nextInt() - 1, sc.nextLong());
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static class DO {
        ...
        public DO(int length, long[] mas) {
            this.length = length;
            this.mas = new long[length*2 - 1];
            if (length >= 0) System.arraycopy(mas, 0, this.mas, length - 1, length);
        ...
        public void set(int i, long value) {
            ...
            while (x != 0) {
                x = (x - 1) / 2;
                mas[x] = Math.min(mas[2*x + 1], mas[2*x + 2]);
            }
            //System.err.println(i + " " + value + " " + Arrays.toString(mas));
        }
        public long min(int l, int r) {
            ...
            while (l <= r) {
                if (l % 2 == 0) {
                    answer = Math.min(answer, mas[l]);
                }
                l = l/2;
                if (r % 2 == 1) {
                    answer = Math.min(answer, mas[r]);
                }
                r = r/2 - 1;
            }
            return answer;
